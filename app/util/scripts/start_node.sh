...
if [ "$1" == "mainnet" ]; then
    ...
    docker run --network="host" --name "tezos-node-$4" tezos/tezos:master tezos-node --cors-header='content-type' --cors-origin='*' --network "$1" --rpc-addr 127.0.0.1:"$2" --net-addr 127.0.0.1:"$3" --history-mode "$5"
fi
